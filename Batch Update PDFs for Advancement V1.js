...
const WITNESS_TIMEOUT_MS = 30000; // 30s max wait for witness cell
const WITNESS_POLL_MS = 400; // poll every 0.4s
const REFRESH_TIMEOUT_MS = 30000; // 30s max wait for data change
const POLL_INTERVAL_MS = 400; // 0.4s poll interval
 ...
 * By updating content, the original Google Drive File ID and URL are preserved.
 */
function createOrUpdateFile_(folder, blob, filename) {
  // 1. Check for existing file by name
  const it = folder.getFilesByName(filename);
  
  if (it.hasNext()) {
    const existingFile = it.next();
    
    // Safety check: delete any other file(s) found with the same name
    while (it.hasNext()) {
      it.next().setTrashed(true);
    }
    
    // 2. Update the content of the primary existing file to preserve the ID/URL
    // We must use getBytes() and setMimeType() when overwriting a non-spreadsheet file type
    return existingFile.setContent(blob.getBytes()).setName(filename).setMimeType(blob.getContentType());

  } else {
    // 3. If no file exists, create a brand new one
